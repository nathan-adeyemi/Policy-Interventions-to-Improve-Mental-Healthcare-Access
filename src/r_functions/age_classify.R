age_classify <- Vectorize(function(x) {
  if (is.na(x)) {
    y <- NA_character_
  } else if (x < 12) {
    y <- 'Child'
    ...
    y <- sample(c('Adolescent','Adult'),1,replace = T)
  } else {
    y <- 'Geriatric'
  }
  return(y)
...
unit_classify <- Vectorize(function(x,
                                    age = NA_real_,
                                    sim_unit_name = FALSE) {
  if (grepl('RST ROGE 03 E BEH|Geriatric/Med Psych', x, ignore.case = T)) {
    if (age >= 65 | is.na(age)) {
      unit <-
        ifelse(test = sim_unit_name,
               yes = "Mayo Rochester Geriatric" ,
               no = 'Geriatric/Med Psych')
    } else{
      unit <-
        ifelse(test = sim_unit_name,
               yes = "Mayo Rochester Adult",
               no = 'Adult Psych')
    }
  } else if (grepl('RST ROGE 01 W BEH|Child/Adolescent Psych', x, ignore.case = T)) {
    unit <-
      ifelse(test = sim_unit_name,
             yes = "Mayo Rochester Pediatric/Adolescent" ,
             no = 'Child/Adolescent Psych')
  } else if (grepl('RST ROGE 02 E BEH|Adult Psych|RST ROGE 03 W BEH|Mood Disorder',
                   x,
                   ignore.case = T)) {
    unit <-
      ifelse(test = sim_unit_name,
...
calc_coordination_time <- function(df) {
  no_availability <-
    df[grepl(
      'no availability|no beds|no capacity|no (.*) availability|no (.*) available|unavailable|no (.*) capacity|no (.*) beds|no (.*) bed|at capacity',
      add_info,
      ignore.case = T
    ) | grepl('\\<full\\>', add_info, ignore.case = T), ]
  ...
  for (i in seq_along(facs)) {
    ...
    if (facs[i] > 1) {
      inds <-
        which(sapply(tolower(df$facility_contacted), function(text)
          test_name == text)) %>% unname() %>% {
            function(i)
              c(i[1], i[length(i)])
          }()
      temp_time = as.numeric(abs(difftime(
        time1 = df[inds[2], event_ts],
...
weekday_factors <- function(data) {
  rbind(data[, .(N = sum(N), day_of_week = unique(day_of_week)), by = Date][, .(AvgAdmits = mean(N)), by = day_of_week],
        data[, .(N = sum(N), day_of_week = unique(day_of_week)), by = Date][, .(AvgAdmits = mean(N))], fill = T)[, factor := AvgAdmits /
                                                                                                                   (.SD[is.na(day_of_week), AvgAdmits])][!is.na(day_of_week)][order(day_of_week)]
}

time_of_day_factors <- function(data) {
  rbind(data[, .(AvgAdmits = mean(N)), by = time_of_day],
        data[, .(AvgAdmits = mean(N))], fill = T)[, factor := AvgAdmits /
                                                    (.SD[is.na(time_of_day), AvgAdmits])][!is.na(time_of_day)]
}
...
boot_confInt_target_val <-
  function(x,
    ...
    p <-
      boot.ci(one.boot(x, FUNCTION, 500),
              conf = interval * 0.01,
              type = interval_type)[[boot_name]]
    endpoint_indices <-
...
boot_confInt_val <- function(x,
  ...
  if (length(x) == 0) {
    ...
    result <-
      one.boot(x, FUNCTION = FUNCTION , R = 1000) %>%  boot.ci(conf = conf.level, type = boot.interval.type)
    if (!is.null(result) & !mean.only) {
      ...
      result <-
        list(conf.int = result[[boot.interval.type]][, c(4, 5)])
      # %>% as.character() %>%
...
boot_confInt_inputs <- function(x,
  ...
  if (length(unique(x)) == 1 & length(x) > 10) {
    ...
    result <-
      one.boot(x, FUNCTION = FUNCTION , R = 1000) %>%  boot.ci(conf = interval /
                                                                 100, type = 'basic')
    if (!is.null(result)) {
      if (mean.only) {
        result <- result$t0
        ...
        result <-
          result$basic[c(4, 5)]  %>% round(digits = 3) %>% as.character() %>%
          {
            function(x)
              paste0('(', x[1], ",", x[2], ")")
        ...
        result <-
          list(
            't0' = result$t0   %>%  round(digits = 3)  %>% as.character(),
...
dist_fit <- function(data) {
  ...
  if (length(na.omit(data)) >= 10) {
    dist_type <-
      names(which.min(mclapply(test_dists(data), function(i)
        i$aic)))
...
dist_cutoff <- function(max_quant, distribution) {
  data <-
    inpatient_stays[inpatient_stays < quantile(inpatient_stays, max_quant)]
