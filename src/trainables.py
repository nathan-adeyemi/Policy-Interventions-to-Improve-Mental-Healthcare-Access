...
def trainable(
    ...
    if ray:
        ...
        if not params and not trainable_path:
            trainable_path = "trainable-Baseline"
        elif not trainable_path:
            trainable_path = f"trainable-{ generate_random_string()}"
    
    trainable_path = os.path.join(scratch_path,trainable_path)
    ...
    subprocess_env.update(
        {
            "port": str(port),
            "seed": str(seed),
            "sim_config": str(fn_cfg_sel),
            "sim_config_path": str(fn_cfg_path),
            "nproc": str(sim_cpus),
            "trainable_name": str(trainable_name),
            "trainable_path": str(trainable_path),
    ...
    return res_dict.get(optim_stat) if isinstance(res_dict,dict) else res_dict
