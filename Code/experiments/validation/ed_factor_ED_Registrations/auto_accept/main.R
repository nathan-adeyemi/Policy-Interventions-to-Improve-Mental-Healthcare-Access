...
valid_list_path <- file.path('Results','validation','ed_factor_ED_Registrations','auto_accept')
...
results <- MH.Network.sim(rep = numIters,
                          ...
                          sort_by_prob = FALSE,
                          input_allocation = NULL,
                          age_frequency_input = NULL,
                          acceptance_prob_input = unique(siteInfo[,list(Facility_name,Acceptance_Prob)])[,Acceptance_Prob := 1], # Sets all acceptance probabilities to 1
