...
if (!any(sapply(
  c("ed_bh_admits", "ed_transfers", "transferred_admits"),
  exists
))) {
  source(
    file.path(".",
      "Data Analysis",
      "Read and Format Mayo Data.R"
    )
  )
}
...
# ip_interarrival_tests <- lapply(c('day_of_week', 'month', 'timeOfDay'),
#                             function(param)
#                               return(summary(with(
#                                 ip_interarrival_test_data, aov(as.formula(paste0('diff ~ ', param,' + location_description')))
#                               ))))
...
mayo_interarrival_rates =
  rbindlist(list(
    rbind(
      ed_interarrival_test_data[, .(AvgAdmits = (mean(remove_outliers(diff), na.rm = T))^
    ...
    rbind(
      ip_interarrival_test_data[ed_patient == F, .(AvgAdmits = (mean(remove_outliers(timediff), na.rm = T))^
...
# buffers =
#   ed_transfers[, .(values = abs(as.numeric(diff(unique(
#     event_ts
#   )))) / 60), by = patient_number]
# buffers_params = dist_fit(buffers[, values])

# Internal Admit Review Time ----------------------------------------------
# self_coord_estimates = dist_fit(ed_bh_admits[, disp_to_assign])
self_coord_mean =
  one.boot(ed_bh_admits[, disp_to_assign], FUN = mean, R = 1000)

self_coord_estimates = dist_fit(data = c(remove_zeros(data = na.omit(
  object = remove_outliers(x = admit_beds[, bed_assign_timespan])
))))
...
# Calculating facility coordination time means and distributions (individual hospitals)----------
coord_times =
  unique(setorder(ed_transfers[!is.na(facility_contacted) & !grepl(paste0(
    "no availability|no beds|",
    "no capacity|no (.*) availability|no (.*) available|unavailable|",
    "no (.*) capacity|no (.*) beds|no (.*) bed|at capacity"),
    add_info,
  ...
  [, .(review_time = abs(difftime(event_ts,data.table::shift(event_ts,1),units = 'hours'))),
    by = list(patient_number, actual_contact)][!is.na(review_time),])
  ...
  #   t2_v2 = max(.SD[call_outcome %in% c("Patient declined by facility", "Patient declined by facility"), event_ts])
  # ),
  # by = list(patient_number, actual_contact)
  # ][, .(review_time = {Vectorize(time_extract)}(t1, t2_v1, t2_v2)),
  #   by = list(actual_contact, patient_number)][!is.na(review_time), ])
...
bed_prep_params <- fitdist(remove_outliers(ed_bh_admits[,`:=`(gap = abs(request_to_departt - request_to_assigned)/60)][,gap]),distr = 'exp')
  # fitdist(data = remove_outliers(as.numeric(ed_transfers[grepl('accept', call_outcome)
  #                                        ][, .(gap = abs(difftime(ed_departure, event_ts, units = 'hours')))
  #                                          ][, gap])), 
  #         distr = 'exp')

overall_avg = coord_times[, one.boot(review_time, mean, R = 1000)$t0]
...
siteInfo <-                                                                                                                                                                                                                                                                                                            
  siteInfo[,lowered_name := tolower(Facility_name)
               ...
               ][,`:=`(Review_Params = overall_dist)
                 ][,Acceptance_Prob := fac_accept_probs[,Acceptance_Prob
                                                        ][match(siteInfo$Facility_name,fac_accept_probs$facility_contacted)],
                   ][is.na(Review_Info), `:=`(Review_Params = overall_dist,
                                              Review_Info = overall_avg)
                     # ][, Acceptance_Prob := Acceptance_Prob #Assign every facility with the same probability of acceptance
                      ][Age == 'Adult',Acceptance_Prob := adult_penalty * Acceptance_Prob
                        ][Age != 'Adult',Acceptance_Prob := all_penalty * Acceptance_Prob
                           ][is.na(Review_Params), Review_Params := overall_dist
...
# Assigning Request Review Times by Individual Hospital
# siteInfo <-                                                                                                                                                                                                                                                                                                            
#   siteInfo[,lowered_name := tolower(Facility_name)
#            ][fac_coord_times, `:=`(Review_Info = Avg), on = c("lowered_name == facility_contacted")
#              ][fac_coord_times, `:=`(Review_Params = Dist), on = c("lowered_name == facility_contacted"), nomatch = list(NA)
#                ][fac_accept_probs, Acceptance_Prob := Acceptance_Prob, on = c("lowered_name == facility_contacted")
#                  ][is.na(Review_Info), `:=`(Review_Params = overall_dist,
#                                             Review_Info = overall_avg)
#                   ][, Acceptance_Prob := .9 * overall_acceptance_prob #Assign every facility with the same probability of acceptance
#                     ][Age == 'Adult',Acceptance_Prob := adult_penalty * Acceptance_Prob
#                      ][is.na(Review_Params), Review_Params := overall_dist
#                        ][,lowered_name := NULL]
# Add the correct HCCIS Year to the Function Requirements -----------------
hccis =
  data.table(readRDS(file = file.path(".",
                                      "Data",
                                      "HCCIS",
                                      'hccis.rds')))
distance.matrix =
